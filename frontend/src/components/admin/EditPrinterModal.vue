      ...
      <div class="bg-white dark:bg-gray-900 rounded-[3rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
        <div class="p-6 md:p-10 max-h-[90vh] overflow-y-auto no-scrollbar">
          <div class="flex justify-between items-center mb-6 md:mb-8">
            <h3 class="text-xl md:text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">Detalles de Máquina</h3>
            <button class="text-gray-400 hover:text-gray-600 transition-colors text-xl md:text-2xl shrink-0 ml-4" @click="emit('update:modelValue', false)">
              ✕
            </button>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 md:gap-4">
            ...
            <button class="sm:flex-[2] py-4 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/20 hover:-translate-y-0.5 transition-all" @click="emit('update-printer')">Guardar Cambios</button>
          </div>
        </div>
      </div>
    </div>
