...
const overduePrintersCount = computed(() => {
  return props.printers.filter(p => isMaintenanceOverdue(p)).length
})
</script>
  ...
  <div class="animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-16 px-6">
      ...
      <div class="flex gap-4 w-full md:w-auto">
        <button @click="$emit('sync')" :disabled="loading" class="p-4 md:p-5 bg-white dark:bg-gray-800 rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-xl text-gray-400 dark:text-gray-300">
          <svg xmlns="http://www.w3.org/2000/svg" :class="`h-5 w-5 md:h-6 md:w-6 ${loading ? 'animate-spin' : ''}`" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
        </button>
        <button @click="$emit('add-printer')" class="btn-primary flex-1 md:flex-none px-6 md:px-10 py-4 md:py-5">
    ...
    <div v-if="overduePrintersCount > 0" 
         class="mx-6 mb-12 p-8 bg-rose-50 border-2 border-rose-100 rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-6 animate-zoom-in">
        ...
        <div class="flex items-center gap-3">
            <div class="flex -space-x-4">
                <div v-for="p in printers.filter(isMaintenanceOverdue).slice(0, 3)" :key="p.id" 
            ...
            <p v-if="overduePrintersCount > 3" class="text-[9px] font-black text-rose-300 uppercase pl-2">+{{ overduePrintersCount - 3 }} más</p>
        </div>
    </div>

    <!-- Machines Grid -->
    <div v-if="printers.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 px-4 pb-12">
      <div v-for="(printer, index) in printers" :key="printer.id" 
        :class="['bg-white dark:bg-gray-900 p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border border-gray-100 dark:border-white/5 shadow-sm group relative overflow-hidden flex flex-col h-full animate-slide-up transition-all hover:scale-[1.02] hover:shadow-2xl', 'animate-stagger-' + (index % 4 + 1)]"
      >
        ...
        <div class="flex items-start justify-between mb-8 md:mb-10">
          <div class="flex gap-6">
            ...
            <div>
              ...
              <div v-if="isMaintenanceNear(printer)" class="inline-flex items-center gap-2 px-3 py-1.5 bg-rose-500 text-white rounded-xl text-[8px] md:text-[9px] font-black uppercase tracking-widest animate-pulse shadow-lg shadow-rose-500/20">
                Revision Urgente
              </div>
            </div>
          </div>
          <div class="flex flex-col items-end">
            <span :class="['px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-white shadow-2xl', getStatusColor(printer.status), printer.status === 'printing' ? 'animate-pulse' : '']">
        ...
        <div class="flex-1 space-y-6 md:mb-10">
          <div class="bg-gray-50/80 dark:bg-gray-950/40 backdrop-blur-sm p-6 md:p-8 rounded-[2.5rem] md:rounded-[3rem] border border-gray-100/50 dark:border-white/5 space-y-6">
                <div class="space-y-4">
                    ...
                    <div class="h-3 bg-white dark:bg-gray-800 rounded-full overflow-hidden shadow-inner p-0.5 border border-gray-100 dark:border-white/5 relative">
                        <div :class="[
                    ...
                    <p class="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase text-right tracking-widest mt-1 italic">Calendario: {{ printer.next_maintenance || 'Sin fecha' }}</p>
                </div>
            ...
            <div class="pt-4 space-y-4">
                ...
                <div class="flex gap-2 p-1.5 bg-white dark:bg-gray-950 rounded-[1.5rem] shadow-sm border border-gray-100 dark:border-white/10">
                    <button 
                        @click="$emit('update-printer-status', { id: printer.id, status: 'idle' })"
                        :class="[printer.status === 'idle' ? 'bg-emerald-500 text-white' : 'text-gray-400 dark:text-gray-300']"
                        class="flex-1 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest"
                    >
                        Libre
                    </button>
                    <button 
                        @click="$emit('update-printer-status', { id: printer.id, status: 'printing' })"
                        :class="[printer.status === 'printing' ? 'bg-amber-500 text-white' : 'text-gray-400 dark:text-gray-300']"
                        class="flex-1 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest"
                    >
                        Ocupada
                    </button>
                    <button 
                        ...
                        :class="[printer.status === 'maintenance' ? 'bg-rose-500 text-white' : 'text-gray-400 dark:text-gray-300']"
                        class="flex-1 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest"
                    >
                        MANT.
                    </button>
                    <button 
                        ...
                        :class="[printer.status === 'offline' ? 'bg-slate-400 text-white' : 'text-gray-400 dark:text-gray-300']"
                        class="flex-1 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest"
                    >
                        OFF
                    </button>
        ...
        <div class="flex flex-col gap-3 pt-6 border-t border-gray-50">
          <button 
            ...
            class="w-full py-4 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center gap-2 group"
          >
          ...
          <div class="flex gap-3">
            <button @click="$emit('edit-printer', printer)" class="btn-primary flex-1 py-4">
              Configurar
            </button>
            <button @click="$emit('delete-printer', printer.id)" class="icon-btn icon-btn-danger w-12 h-12">
    ...
    <div v-else class="flex flex-col items-center justify-center py-20 md:py-32 px-6 animate-zoom-in">
        <div class="w-24 h-24 md:w-32 md:h-32 bg-gray-100 dark:bg-white/5 rounded-[2.5rem] md:rounded-[3rem] flex items-center justify-center text-5xl md:text-6xl mb-8 md:mb-10 shadow-inner">
        ...
        <button @click="$emit('add-printer')" class="btn-primary px-12 py-5 shadow-2xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all">
