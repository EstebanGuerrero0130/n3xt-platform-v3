...
const emit = defineEmits<{
  ...
  (e: "add-material", data: any): void
}>()

// Copia local del formulario — completamente independiente del padre
const form = reactive({
  id: '', code: '', name: '', category: 'FDM', type: 'material',
  unit: 'g', cost_per_kg: 0, density: 1.24, color: '#000000',
  initial_stock: 1000, low_stock_threshold: 200,
  package_price: 0, package_qty: 0, package_units: 1
})

// Solo sincroniza del padre al local cuando el modal se abre
watch(() => props.modelValue, (isOpen) => {
  if (isOpen && props.newMaterial) {
    Object.assign(form, props.newMaterial)
  }
})

const submitForm = () => {
  emit('add-material', { ...form })
}
</script>
...
<template>
  ...
  <button class="w-10 h-10 md:w-12 md:h-12 shrink-0 bg-[#151a22] dark:bg-[#151a22]/5 text-[#c3c4c5] rounded-[24px] flex items-center justify-center hover:bg-[#151a22] dark:hover:bg-[#151a22]/10 transition-all text-lg md:text-xl" aria-label="Cerrar" @click="emit('update:modelValue', false)">
  ✕
  </button>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-10">
    ...
    <input v-model="form.id" type="text" placeholder="Ej: MAT-PLA-001" required class="w-full bg-[#151a22] dark:bg-[#151a22]/5 border border-primary/20 rounded-[24px] p-5 font-black text-sm text-[#ffffff] dark:text-white outline-none focus:ring-2 focus:ring-primary transition-all">
    </div>
  ...
  $ {{ (form.package_price / ((form.package_qty || 1) * (form.package_units || 1))).toLocaleString(undefined, {minimumFractionDigits: 2}) }} / {{ form.unit }}
  <span v-if="form.unit === 'g' || form.unit === 'ml'" class="text-[8px] opacity-70 ml-1">
  ($ {{ ((form.package_price / ((form.package_qty || 1) * (form.package_units || 1))) * 1000).toLocaleString(undefined, {maximumFractionDigits: 0}) }} x Kg/L)
  </span>
  </p>
  </div>
  </div>
