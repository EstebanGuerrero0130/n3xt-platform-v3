<template>
  ...
  <option v-for="m in (materials as any[])" :key="m.id" :value="m.id">{{ m.name }}</option>
  </select>
  <div class="flex flex-col sm:flex-row gap-2">
...
const materialName = computed(() => {
  ...
  const mat = (props.materials as any[]).find(m => String(m.id) === String(props.order.material_id))
...
const breakdown = computed(() => {
  ...
  const matCostPerKg = props.order.cost_snapshot?.material_cost_per_kg
    || (props.materials as any[]).find(m => String(m.id) === String(props.order.material_id))?.cost_per_kg
    || 0
...
const sendPdfEmail = async () => {
  ...
  try {
    await api.post(`/admin/orders/${props.order.id}/email`, {})
  } catch (err) {
