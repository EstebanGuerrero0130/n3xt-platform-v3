<template>
 ...
 <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-8 mb-8 md:mb-16 bg-[#151a22]/40 dark:bg-[#090d0a]/40 backdrop-blur-xl p-4 md:p-12 rounded-[2rem] md:rounded-[4rem] border border-white/60 dark:border-[#21262d] -gray-200/30 dark:-none relative overflow-hidden group">
 ...
 <h2 class="text-xl md:text-4xl font-black text-[#ffffff] dark:text-white tracking-tighter uppercase mb-2 md:mb-4">Configuración <span class="text-[#8dd6ff]">Global</span></h2>
 <p class="text-[9px] md:text-sm text-[#c3c4c5] font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] flex items-center gap-2 md:gap-3">
 <span class="w-2 h-2 md:w-3 md:h-3 rounded-[60px] bg-emerald-500 animate-pulse"></span>
 Motor de Gestión Financiera • N3XT Engine
 </p>
 </div>

 <button class="bg-[#08872b] hover:bg-emerald-600 text-white w-full md:w-auto px-6 md:px-12 py-4 md:py-6 rounded-[1.5rem] md:rounded-[2rem] -primary/20 flex items-center justify-center gap-3 md:gap-4 active:scale-95 transition-all duration-200 z-50 cursor-pointer text-sm md:text-lg" @click="() => saveSettings()">
 <span v-if="savingSettings" class="w-4 h-4 md:w-6 md:h-6 border-3 md:border-4 border-white/30 border-t-white rounded-[60px] animate-spin"></span>
 ...
 <!-- Branding Card -->
 <div class="bg-[#151a22] p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] border border-gray-700/50 -black/30 relative overflow-hidden group">
 ...
 <div class="absolute w-2.5 h-2.5 bg-[#151a22]/30 rounded-[60px] top-3 left-3 animate-ping"></div>
 <div class="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent"></div>
 <div class="relative z-10 flex flex-col items-center">
 <span class="text-white font-black italic text-4xl tracking-tighter drop--[0_4px_8px_rgba(0,0,0,0.4)]">N</span>
 <span class="text-[8px] font-black text-white/50 uppercase tracking-[0.3em] mt-0.5">N3XT</span>
 </div>
 <div class="absolute -bottom-2 -right-2 w-12 h-12 bg-emerald-400/10 rounded-[60px] blur-xl animate-pulse" style="animation-delay: 0.5s"></div>
 </div>
 ...
 class="bg-[#151a22] dark:bg-[#08872b] text-white px-12 py-5 rounded-[24px] text-[10px] font-black uppercase tracking-[0.2em] hover:scale-105 active:scale-95 transition-all duration-300 disabled:opacity-50 disabled:cursor-wait flex items-center gap-3"
 @click="($refs.logoInput as HTMLInputElement).click()"
 >
 <span v-if="uploadingLogo" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-[60px] animate-spin"></span>
 ...
 <label class="text-[10px] font-black text-[#c3c4c5] uppercase tracking-widest ml-5 mb-2 block group-focus-within/input:text-[#8dd6ff] transition-colors">Nombre Oficial</label>
 <input v-model="localSettings.company.name" type="text" class="w-full bg-[#283041] border-2 border-transparent rounded-[2rem] px-8 py-5 text-sm font-bold text-white outline-none focus:bg-gray-700 focus:border-primary/20 focus: transition-all duration-300">
 </div>
 <div class="group/input">
 <label class="text-[10px] font-black text-[#c3c4c5] uppercase tracking-widest ml-5 mb-2 block group-focus-within/input:text-[#8dd6ff] transition-colors">Slogan Corporativo</label>
 <input v-model="localSettings.company.slogan" type="text" class="w-full bg-[#283041] border-2 border-transparent rounded-[2rem] px-8 py-5 text-sm font-bold text-white outline-none focus:bg-gray-700 focus:border-primary/20 focus: transition-all duration-300">
 </div>
 </div>
 ...
 <!-- Danger Zone -->
 <div class="bg-rose-950/30 p-8 md:p-12 rounded-[3rem] border border-rose-900/30 -rose-950/30 relative overflow-hidden group">
 <div class="relative z-10 flex flex-col items-center">
 ...
 <!-- Master Costs Card -->
 <div class="bg-[#151a22] p-8 md:p-16 rounded-[4rem] border border-gray-700/50 -black/40 relative group">
 ...
 <p class="text-[10px] md:text-xs font-bold text-[#c3c4c5] uppercase tracking-[0.4em] mt-2">Algoritmo de Cálculo Operativo</p>
 </div>
 <div class="flex items-center gap-4 px-6 py-3 bg-emerald-900/30 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest animate-pulse border border-emerald-700/30 -emerald-500/5">
 <span class="w-2.5 h-2.5 rounded-[60px] bg-emerald-500 -[0_0_10px_rgba(16,185,129,0.5)]"></span>
 ...
 <div class="w-12 h-12 rounded-[24px] bg-rose-500/10 text-rose-500 flex items-center justify-center ">
 ...
 <p class="text-[10px] font-bold text-[#c3c4c5] uppercase tracking-widest">Costos fijos operacionales</p>
 </div>
 </div>
...
v-model.number="localSettings.infra[key]" type="number" min="0" step="any"
 ...
 <div class="col-span-1 sm:col-span-2 bg-[#151a22] p-8 md:p-10 rounded-[3rem] border border-gray-800 hover:border-primary transition-all duration-700 group relative overflow-hidden -black/40">
 <div class="absolute -right-10 -top-10 w-40 h-40 bg-[#08872b]/10 rounded-[60px] blur-[80px] group-hover:bg-[#08872b]/20 transition-all duration-1000"></div>
 <div class="flex justify-between items-start mb-6 relative z-10">
 <div>
 <p class="text-[10px] font-black text-[#c3c4c5] uppercase tracking-[0.3em] group-hover:text-[#8dd6ff] transition-colors">Factor de Carga Operativa</p>
 <p class="text-[8px] font-bold text-[#a4aea6] uppercase tracking-widest mt-1">Eficiencia Real vs Teórica</p>
 </div>
 <span class="text-[10px] font-black bg-[#08872b]/20 text-[#8dd6ff] px-4 py-1.5 rounded-[60px] border border-primary/20">Modo Alta Precisión</span>
 </div>
 <div class="flex items-center gap-8 relative z-10">
 <input v-model.number="localSettings.infra.load_factor" type="number" step="0.1" min="0" max="1" class="w-28 bg-transparent border-none p-0 font-black text-5xl text-white outline-none tracking-tighter focus:text-[#8dd6ff] transition-all">
 <div class="flex-1 h-3 bg-[#151a22]/5 rounded-[60px] overflow-hidden border border-white/5 p-[2px]">
 <div :style="{ width: (Math.min(Math.max(localSettings.infra.load_factor * 100, 0), 100)) + '%' }" class="h-full bg-[#08872b] transition-all duration-1000 -[0_0_20px_rgba(var(--primary),0.8)] rounded-[60px]"></div>
 </div>
 </div>
 <p class="text-[10px] font-medium text-[#a4aea6] mt-6 leading-relaxed uppercase tracking-widest opacity-80">
 Ajuste de consumo para compensar tiempos muertos. <br>
 <span class="text-[#8dd6ff] font-black">Estándar N3XT: 0.4 (40%)</span>
 </p>
 </div>
 </div>
 </div>
 ...
 <div class="space-y-10">
 <div class="flex items-center gap-4 border-b border-gray-700/50 pb-6">
 <div class="w-12 h-12 rounded-[24px] bg-emerald-500/10 text-emerald-500 flex items-center justify-center ">
 ...
 <p class="text-[10px] font-bold text-[#c3c4c5] uppercase tracking-widest">Riesgo & Utilidad Neta</p>
 </div>
 </div>
 <div class="space-y-6">
 <div class="bg-gradient-to-br from-emerald-950/60 to-gray-900 p-7 rounded-[24px] border border-emerald-900/40 group -emerald-500/5">
 <p class="text-[10px] font-bold text-emerald-300 uppercase mb-1">Margen de Utilidad Objetivo</p>
 <p class="text-[9px] font-medium text-[#a4aea6] mb-4">Retorno sobre costo de producción</p>
 ...
 <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
 <div class="bg-[#283041]/60 p-5 rounded-[24px] border border-gray-700/60 hover:border-primary/40 hover:bg-[#283041]/80 hover: transition-all duration-300 group min-w-0">
 <p class="text-[10px] font-bold text-[#c3c4c5] uppercase mb-3 group-hover:text-[#8dd6ff] transition-colors truncate" title="Mano de Obra ($/Hr)">Mano de Obra ($/Hr)</p>
 <div class="flex items-baseline gap-1 min-w-0">
 <span class="text-[#8dd6ff]/50 font-bold text-base group-hover:text-[#8dd6ff]/70 transition-colors shrink-0">$</span>
 ...
 <p class="text-[9px] text-[#8dd6ff]/40 font-medium mt-2 truncate">{{ defaultSuggestions.mano_obra_hr }}</p>
 </div>
 
 <div class="bg-[#283041]/60 p-5 rounded-[24px] border border-gray-700/60 hover:border-primary/40 hover:bg-[#283041]/80 hover: transition-all duration-300 group min-w-0">
 <p class="text-[10px] font-bold text-[#c3c4c5] uppercase mb-3 group-hover:text-[#8dd6ff] transition-colors truncate" title="% Prep / Post">% Prep / Post</p>
 <div class="flex items-baseline gap-1 min-w-0">
 ...
 <p class="text-[9px] text-[#8dd6ff]/40 font-medium mt-2 truncate">{{ defaultSuggestions.prep_time_pct }}</p>
 </div>

 <div class="bg-[#283041]/60 p-5 rounded-[24px] border border-rose-900/40 hover:border-rose-500/40 hover:bg-[#283041]/80 hover: transition-all duration-300 group min-w-0">
 <p class="text-[10px] font-bold text-[#c3c4c5] uppercase mb-3 group-hover:text-rose-400 transition-colors truncate" title="Fallas & Scrap">Fallas & Scrap</p>
 <div class="flex items-baseline gap-1 min-w-0">
 ...
 <div class="bg-indigo-950/60 p-5 rounded-[24px] border border-indigo-900/50 hover:border-indigo-400/50 transition-all duration-300 group min-w-0">
 ...
 <!-- % Logística -->
 <div class="bg-cyan-950/60 p-5 rounded-[24px] border border-cyan-900/50 hover:border-cyan-400/50 transition-all duration-300 group min-w-0">
 ...
 <!-- % Marketing -->
 <div class="bg-pink-950/60 p-5 rounded-[24px] border border-pink-900/50 hover:border-pink-400/50 transition-all duration-300 group min-w-0">
 ...
 <div class="lg:col-span-2 bg-[#151a22] p-8 md:p-16 rounded-[4rem] relative overflow-hidden h-fit border border-white/5">
 <div class="absolute -right-20 -bottom-20 w-96 h-96 bg-indigo-500/5 rounded-[60px] blur-[120px] pointer-events-none"></div>
 <div class="relative z-10">
 ...
 <label class="text-[10px] font-black text-[#a4aea6] uppercase tracking-[0.2em] ml-6 group-focus-within/fiscal:text-indigo-400 transition-colors">{{ label }}</label>
 <input v-model="localSettings.company[key]" :type="key === 'email' ? 'email' : 'text'" class="w-full bg-[#151a22]/5 border border-white/10 rounded-[2rem] px-8 py-5 text-sm font-bold text-white outline-none focus:bg-[#151a22]/10 focus:border-indigo-500 focus:-[0_0_30px_rgba(99,102,241,0.1)] transition-all duration-300">
 </div>
 </div>
 </div>
