...
const getStatusColor = (status) => {
    switch (status) {
        ...
        case 'delivered': return 'bg-gray-900'
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 md:py-10 md:px-6 relative z-10">
        ...
        <header class="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 bg-white/80 dark:bg-black/60 backdrop-blur-3xl p-6 md:p-10 rounded-[3rem] border border-gray-200 dark:border-white/5 shadow-xl">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 md:w-20 md:h-20 bg-primary rounded-3xl flex items-center justify-center text-white text-3xl shadow-[0_0_20px_#1e3a34] font-black italic">
                    {{ customer?.name?.charAt(0) || 'U' }}
                </div>
                <div>
                    <p class="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-1">Acceso Autorizado</p>
                    <h1 class="text-2xl md:text-4xl font-black text-gray-900 dark:text-white tracking-tighter uppercase italic leading-none">
                        Misión: <span class="text-primary">{{ customer?.name?.split(' ')[0] || 'Maker' }}</span>
                    </h1>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">Mis Impresiones 3D</h2>
                </div>
                ...
                <div v-else-if="orders.length === 0" class="bg-white/40 dark:bg-white/5 backdrop-blur-xl p-20 rounded-[4rem] border border-white dark:border-white/5 text-center shadow-2xl relative overflow-hidden group">
                    ...
                    <h3 class="text-2xl font-black text-gray-900 dark:text-white uppercase mb-3 tracking-tighter">Aún no tienes pedidos</h3>
                    <p class="text-gray-400 dark:text-gray-500 text-sm font-bold uppercase tracking-tight mb-10 leading-relaxed">Sube tu primer diseño y comienza a fabricar<br>con precisión quirúrgica.</p>
                    <router-link to="/quote" class="group relative px-12 py-5 bg-primary text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-2xl shadow-primary/40 hover:scale-105 active:scale-95 transition-all overflow-hidden">
            ...
            <!-- Sidebar / Promotions -->
            <div class="space-y-6">
                <div class="bg-gradient-to-br from-primary to-emerald-600 p-10 rounded-[3rem] text-white shadow-2xl shadow-primary/20 relative overflow-hidden group">
                    ...
                    <div class="relative z-10">
                        ...
                        <router-link to="/quote" class="inline-block bg-white text-primary px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-transform">Ver Promociones</router-link>
                    </div>
                    <div class="absolute -bottom-10 -right-10 w-32 h-32 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all"></div>
                </div>

                <div class="bg-white/60 dark:bg-white/5 backdrop-blur-xl p-8 rounded-[3rem] border border-white dark:border-white/10 shadow-xl">
                    <h4 class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest mb-8">Soporte Técnico</h4>
                    <div class="space-y-4">
                        <a href="https://wa.me/573118796416" target="_blank" class="flex items-center gap-5 p-5 bg-gray-50/50 dark:bg-white/5 rounded-2xl hover:bg-emerald-500/10 transition-all border border-transparent hover:border-emerald-500/20 group">
                            ...
                            <div>
                                <p class="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-widest">WhatsApp Directo</p>
                                <p class="text-[8px] text-emerald-500 font-bold uppercase tracking-widest mt-0.5">Respuesta inmediata</p>
                            </div>
                        </a>
                        <a href="mailto:servicion3xt@gmail.com" class="flex items-center gap-5 p-5 bg-gray-50/50 dark:bg-white/5 rounded-2xl hover:bg-primary/10 transition-all border border-transparent hover:border-primary/20 group">
                            <svg class="w-6 h-6 text-primary group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            <div>
                                <p class="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-widest">Correo Oficial</p>
                                <p class="text-[8px] text-primary font-bold uppercase tracking-widest mt-0.5">servicion3xt@gmail.com</p>
                            </div>
                        </a>
                        <router-link to="/catalog" class="flex items-center gap-5 p-5 bg-gray-50/50 dark:bg-white/5 rounded-2xl hover:bg-gray-500/10 transition-all border border-transparent hover:border-gray-500/20 group">
                            <svg class="w-6 h-6 text-gray-500 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                            <div>
                                <p class="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-widest">Guía de Materiales</p>
                                <p class="text-[8px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">¿Cuál elegir?</p>
                            </div>
                        </router-link>
                    </div>
                </div>
