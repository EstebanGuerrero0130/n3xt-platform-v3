  ...
  <div class="min-h-screen bg-[#f8fafc] dark:bg-[#0a0f14] text-gray-900 dark:text-white transition-colors duration-500 overflow-x-hidden">
    ...
    <main class="max-w-7xl mx-auto px-6 py-20">
      ...
      <div v-if="loading" class="text-center py-40">
        <div class="w-16 h-16 border-4 border-emerald-500/30 border-t-emerald-500 rounded-full animate-spin mx-auto mb-6"></div>
        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Cargando proyecto...</p>
      </div>
      ...
      <div v-else-if="!item" class="text-center py-40 space-y-8">
        ...
        <p class="text-lg font-black text-gray-400 uppercase tracking-widest italic">Proyecto no encontrado</p>
        <router-link to="/galeria" class="inline-flex items-center gap-3 px-10 py-4 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-xl">
          ← Volver a la Galería
        </router-link>
      </div>
      ...
      <template v-else>
        <!-- Back link -->
        <button class="group flex items-center gap-3 text-gray-400 hover:text-emerald-500 transition-colors mb-12 text-[10px] font-black uppercase tracking-widest" @click="router.push('/galeria')">
...
v-if="!mainImageFailed" :src="getOptimizedImage(item.image)" :alt="'Proyecto de impresión 3D: ' + item.title + (item.category ? ' (' + item.category + ')' : '') + ' — N3XT 3D Galería'"
              ...
              <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                <div class="w-16 h-16 bg-white/90 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all transform scale-75 group-hover:scale-100 shadow-2xl">
                  <svg class="w-8 h-8 text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"/></svg>
                </div>
              </div>
            </div>
...
v-for="(img, idx) in allImages" :key="idx"
          ...
          <div class="space-y-8">
            ...
            <div class="flex flex-wrap gap-3">
              <span class="px-4 py-1.5 bg-emerald-500/10 text-emerald-500 text-[8px] font-black rounded-full uppercase tracking-[0.25em] border border-emerald-500/20">{{ item.category }}</span>
              <span v-if="item.technology" class="px-4 py-1.5 bg-black/5 dark:bg-white/5 text-gray-500 text-[8px] font-black rounded-full uppercase tracking-[0.25em] border border-gray-200 dark:border-white/10">{{ item.technology }}</span>
              <span v-if="item.material" class="px-4 py-1.5 bg-black/5 dark:bg-white/5 text-gray-500 text-[8px] font-black rounded-full uppercase tracking-[0.25em] border border-gray-200 dark:border-white/10">{{ item.material }}</span>
            </div>

            <!-- Title -->
            <div>
              <h1 class="split-title text-5xl md:text-7xl font-black tracking-tighter uppercase leading-[0.9]">{{ item.title }}</h1>
              <p v-if="item.subtitle" class="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mt-3">{{ item.subtitle }}</p>
            </div>

            <!-- Description -->
            <p v-if="item.description" class="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-bold uppercase leading-relaxed tracking-[0.3em]">
              {{ item.description }}
            </p>

            <!-- Client -->
            <div v-if="item.client" class="flex items-center gap-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
              <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Cliente: <span class="text-gray-900 dark:text-white">{{ item.client }}</span></span>
            </div>

            <!-- Date -->
            <div v-if="item.date" class="flex items-center gap-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
              <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Fecha: <span class="text-gray-900 dark:text-white">{{ formatDate(item.date) }}</span></span>
            </div>

            <!-- Dimensions -->
            <div v-if="item.dimensions" class="flex items-center gap-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4"/></svg>
              <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Dimensiones: <span class="text-gray-900 dark:text-white">{{ item.dimensions }}</span></span>
            </div>

            <!-- Tags -->
            <div v-if="normalizeTags(item.tags).length > 0" class="flex flex-wrap gap-2">
              <span v-for="tag in normalizeTags(item.tags)" :key="tag" class="px-4 py-2 bg-gray-100 dark:bg-white/5 text-gray-500 text-[8px] font-black rounded-full uppercase tracking-[0.15em] border border-gray-200 dark:border-white/10">{{ tag }}</span>
            </div>
...
v-for="stat in item.stats" :key="stat.label"
                   class="bg-gray-50 dark:bg-white/[0.03] rounded-2xl p-5 text-center border border-gray-100 dark:border-white/5">
                <p class="text-2xl font-black text-gray-900 dark:text-white">{{ stat.val }}</p>
                <p class="text-[8px] font-bold text-gray-400 uppercase tracking-[0.15em] mt-1">{{ stat.label }}</p>
              </div>
            </div>
            ...
            <div class="flex flex-wrap gap-4 pt-4">
              <router-link
to="/quote"
                   class="split-btn px-10 py-5 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20 hover:scale-[1.02] active:scale-95">
                Cotizar Pieza Similar →
              </router-link>
              <router-link
to="/catalog"
                   class="px-10 py-5 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:border-emerald-500/30 transition-all">
                Explorar Catálogo
              </router-link>
            </div>
          </div>
...
v-for="(related, idx) in webSettings.gallery.filter(g => g.title !== item.title).slice(0, 3)" :key="idx"
              ...
              :class="['group bg-white dark:bg-white/5 rounded-[2rem] overflow-hidden border border-gray-100 dark:border-white/10 hover:border-emerald-500/30 transition-all hover:shadow-xl', relatedVisible ? 'related-revealed' : 'related-hidden']"
...
:src="getOptimizedImage(related.image)" :alt="'Proyecto relacionado: ' + related.title + ' — N3XT 3D Galería'"
              ...
              <div class="p-6 text-center">
                ...
                <p class="text-[8px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1">{{ related.category }}</p>
              </div>
            </router-link>
          </div>
        </div>
...
v-if="lightboxOpen && allImages.length" ref="lightboxRef" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12"
        ...
        <!-- Close -->
        <button
class="absolute top-6 right-6 z-10 w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-2xl flex items-center justify-center transition-all backdrop-blur-xl border border-white/10"
                @click="lightboxOpen = false">
          <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>

        <!-- Counter -->
        <div class="absolute top-6 left-6 z-10 px-4 py-2 bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 text-white text-[10px] font-black tracking-widest">
          {{ lightboxIdx + 1 }} / {{ allImages.length }}
        </div>

        <!-- Prev -->
        <button
v-if="lightboxIdx > 0" class="absolute left-6 z-10 w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-2xl flex items-center justify-center transition-all backdrop-blur-xl border border-white/10"
                @click="prevImage">
        ...
        <!-- Next -->
        <button
v-if="lightboxIdx < allImages.length - 1" class="absolute right-6 z-10 w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-2xl flex items-center justify-center transition-all backdrop-blur-xl border border-white/10"
                @click="nextImage">
