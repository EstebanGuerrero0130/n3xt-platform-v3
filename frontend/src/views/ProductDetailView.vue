  ...
  <div class="min-h-screen bg-[#f8fafc] dark:bg-[#0a0f14] text-gray-900 dark:text-white transition-colors duration-500 overflow-x-hidden">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12 md:py-20">
      ...
      <template v-else-if="product">
        ...
        <!-- Columna Galería -->
        <div class="space-y-6 animate-in fade-in slide-in-from-left-4 duration-1000">
          <div class="aspect-square bg-gray-50 dark:bg-gray-900 rounded-[3rem] overflow-hidden border border-gray-100 dark:border-white/5 shadow-2xl relative group p-10 flex items-center justify-center">
            <img :src="product.image" :alt="'Detalle de producto: ' + product.name" class="max-w-full max-h-full object-contain group-hover:scale-105 transition-transform duration-[2s]" />
          ...
          <!-- Miniaturas (Mockup) -->
          <div class="grid grid-cols-4 gap-4">
             <div v-for="i in 4" :key="i" class="aspect-square bg-gray-100 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden cursor-pointer hover:border-primary transition-all">
                <img :src="product.image" :alt="'Vista previa ' + i + ' de ' + product.name" class="w-full h-full object-cover opacity-50 hover:opacity-100 transition-opacity" />
             </div>
          </div>
        </div>
        ...
        <div class="space-y-10 text-left animate-in fade-in slide-in-from-right-4 duration-1000">
          ...
          <div class="pt-10 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row gap-5">
              <button @click="checkoutWhatsApp" class="flex-1 px-10 py-6 bg-emerald-500 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] shadow-3xl shadow-emerald-500/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 group">
              ...
              <button @click="checkoutWhatsApp" class="flex-1 px-10 py-6 bg-gray-950 dark:bg-white/5 border border-transparent dark:border-white/10 text-white dark:text-gray-300 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] hover:bg-primary hover:text-white transition-all flex items-center justify-center">
                Consultar Protocolo
              </button>
            </div>
